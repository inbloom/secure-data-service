...
public class EdfiRecordParserImpl2 extends DefaultHandler {
    ...
    private RecordMeta getRecordMetaForEvent(String localName) {
        RecordMeta typeMeta = typeProvider
                .getTypeFromParentType(complexTypeStack.peek().getLeft().getType(), localName);

        if (typeMeta == null) {
            typeMeta = new RecordMetaImpl(localName, "UNKNOWN");
        }
        return typeMeta;
    }

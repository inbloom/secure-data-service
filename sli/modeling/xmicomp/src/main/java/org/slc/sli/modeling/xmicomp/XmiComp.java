...
public final class XmiComp {
    ...
    public static void main(final String[] args) {
        ...
        final OptionSpec<File> mappingSpec = optionSpec(parser, ARG_MAP_FILE, "Mapping (input) file", File.class);
        ...
        try {
            ...
            if (options.hasArgument(helpSpec)) {
                ...
                try {
                    ...
                    final XmiMappingDocument mapping = XmiMappingReader.readDocument(mappingFile);
    ...
    private static final void checkMappingDocument(final XmiMappingDocument mappingDocument, final ModelIndex lhsModel,
            final ModelIndex rhsModel) {
    ...
    private static final void checkMapping(final XmiMappingDocument mappingDocument, final XmiMapping mapping,
        ...
        if (lhsFeature != null) {
            checkFeature(mappingDocument.getLhsModel(), lhsFeature, lhsModel, status);
        }
        final XmiFeature rhsFeature = mapping.getRhsFeature();
        if (rhsFeature != null) {
            checkFeature(mappingDocument.getRhsModel(), rhsFeature, rhsModel, status);
        }
    }

    private static final void checkFeature(final XmiMappingModel context, final XmiFeature feature,
        ...
        if (model.getClassTypes().containsKey(type)) {
            ...
            System.err.println(type + " in " + context.getName() + " is not a recognized type. status : " + status);
        }
    }
}

...
public final class Xsd2UmlConvert {
        ...
        if (schemaType == null || annotation == null) {
            ...
            for (int childIdx = 0; childIdx < children.getCount(); ++childIdx) {
                ...
                if (child instanceof XmlSchemaDocumentation) {
                    ...
                    taggedValues.addAll(config.convertAppInfo(appInfo, config));
                } else {
    ...
    private static final void convertComplexType(final XmlSchemaComplexType complexType, final XmlSchema schema,
        ...
        if (complexType.getContentModel() != null && complexType.getContentModel().getContent() != null) {
            ...
            if (content instanceof XmlSchemaComplexContentExtension) {
                ...
                // FIXME: Hack here to support anonymous complex types in the context of elements.
                // FIXME: Need to fix the SLI MongoDB schemes so that all types are named.
                handler.visit(new Generalization(config.getGeneralizationNameForComplexTypeExtension(complexTypeName,
                        base), complexTypeId, baseId));
        ...
        handler.visit(new ClassType(complexTypeId, config.convertTypeName(complexTypeName), false, attributes,
                taggedValues));
    }
    ...
    private static final void convertSimpleType(final XmlSchemaSimpleType simpleType, final XmlSchema schema,
        ...
        if (content instanceof XmlSchemaSimpleTypeList) {
            ...
            if (enumLiterals.isEmpty()) {
                final DataType dataType = new DataType(simpleTypeId, config.convertTypeName(simpleType.getQName()),
                        false, taggedValues);
                handler.visit(dataType);
                typeIdentifier = dataType.getId();
                if (!restriction.getBaseTypeName().equals(WxsNamespace.STRING)) {
                    handler.visit(new Generalization(config.getGeneralizationNameForSimpleTypeRestriction(
                            simpleType.getQName(), restriction.getBaseTypeName()), typeIdentifier, baseId));
                }
            } else {
                final EnumType enumType = new EnumType(simpleTypeId, config.convertTypeName(simpleType.getQName()),
    ...
    private static final EnumLiteral enumLiteralFromFacet(final XmlSchemaEnumerationFacet enumFacet,
        ...
        // We're assuming here that the whiteSpace facet for this enumeration type is "collapse".
     ...
     */
    public static final Model extractModel(final String name, final int level, final XmlSchema schema,
            final Xsd2UmlPlugin plugin) {
        final Xsd2UmlConfig context = new Xsd2UmlConfig(plugin);
        return parseXmlSchema(name, level, schema, context);
    }
    ...
    private static final Attribute parseElement(final XmlSchemaElement element, final XmlSchema schema,
        ...
        final String name = config.convertAttributeName(element.getQName());
        final List<TaggedValue> taggedValues = new LinkedList<TaggedValue>();
    ...
    private static final Model parseXmlSchema(final String name, final int level, final XmlSchema schema,
        ...
        if (level == 0) {
            for (final TagDefinition tagDefinition : declareTagDefinitions(context)) {
                ownedElements.add(tagDefinition);
            }
            final List<DataType> dataTypes = declareBuiltInDataTypes(context);
        ...
        for (int i = 0, count = schemaItems.getCount(); i < count; i++) {
            ...
            if (schemaObject instanceof XmlSchemaType) {
                ...
                convertType(schemaType, schema, context, new Visitor() {
                ...
                }, schemaType.getQName(), annotations(schemaType, context));
                ...
                if (element.getSchemaTypeName() == null) {
                    ...
                    convertType(elementType, schema, context, new Visitor() {
                    ...
                    }, element.getQName(), annotations(element, context));
                }
                ...
                final Model embeddedModel = parseXmlSchema("", level + 1, embeddedSchema, context);

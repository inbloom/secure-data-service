 ...
 */
public final class Uml2XsdCmdLine {
    ...
    private static final PsmConfig<Type> extractConfig(final Mapper mapper) {
        ...
        for (final ClassType classType : mapper.getClassTypes()) {
            ...
            for (final PsmResource resource : getResources(taggedValues, mapper)) {
                ...
                classTypes.add(new PsmClassType<Type>(classType, resource, collection));
            }
        }
        return new PsmConfig<Type>(classTypes);
    }

    private static final List<PsmResource> getResources(final List<TaggedValue> taggedValues, final Mapper mapper) {
        ...
        for (final TaggedValue taggedValue : taggedValues) {
            ...
            final TagDefinition tagDefinition = mapper.getTagDefinition(tagDefinitionId);

...
import org.slc.sli.ingestion.xml.idref.SnippetFile.SnippetBlock;
        ...
        TestExecutor findMatchingEntities = new TestExecutor() {
            ...
            public void execute(File file) throws Throwable {
                ...
                try {
                    ...
                    result = (SnippetFile) PrivateAccessor.invoke(idRefResolutionHandler, "findMatchingEntities", new Class[]{File.class, Set.class, ErrorReport.class}, new Object[]{file, refs, errorReport});

                    Assert.assertNotNull(result);
                    Map<String, SnippetBlock> snippets = (Map<String, SnippetBlock>) PrivateAccessor.getField(result, "snippets");
                    Assert.assertEquals(1, snippets.size());
                    Assert.assertTrue(snippets.containsKey("GBE-8th Grade English-3"));
                } finally {
                    if (result != null) {
                        result.delete();
                    }

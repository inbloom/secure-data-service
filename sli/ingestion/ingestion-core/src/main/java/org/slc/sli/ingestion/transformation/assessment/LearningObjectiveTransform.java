...
public class LearningObjectiveTransform extends AbstractTransformationStrategy {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(LearningObjectiveTransform.class);
	...
	protected void performTransformation() {
		...
		LOG.info("{} is loaded into local storage.  Total Count = {}",
				LEARNING_OBJECTIVE, learningObjectiveIdMap.size());
		...
		for (NeutralRecord parentLO : learningObjectives) {
			...
			flipLearningObjectiveRelDirection(parentLO, learningObjectiveIdMap,
					transformedLearningObjectives);
	...
	private Map<LearningObjectiveId, NeutralRecord> prepareLearningObjectiveLookupMap(
		...
		for (NeutralRecord lo : learningObjectives) {
			...
			String objectiveGradeLevel = getByPath("objectiveGradeLevel",
					attributes);
			if (objective != null && academicSubject != null
					&& academicSubject != null) {
				if (learningObjectiveIdMap.containsKey(new LearningObjectiveId(
						objective, academicSubject, objectiveGradeLevel))) {
					super.getErrorReport(lo.getSourceFile())
							.error("Two or more LearningObjectives have duplicate IdentificationCode, ContentStandardName combination. Objective: "
									+ objective
									+ ", AcademicSubject"
									+ academicSubject
									+ ", ObjectiveGradeLevel"
				...
				learningObjectiveIdMap.put(new LearningObjectiveId(objective,
						academicSubject, objectiveGradeLevel), lo);
			}
		}
	...
	private void flipLearningObjectiveRelDirection(NeutralRecord parentLO,
		...
		List<Map<String, Object>> childLearningObjRefs = (List<Map<String, Object>>) parentLO
				.getAttributes().get(LEARNING_OBJ_REFS);
		...
		for (Map<String, Object> childLORef : childLearningObjRefs) {
			...
			if (childLearningObjRefs != null) {
				String objective = getByPath(
						"LearningObjectiveIdentity.Objective", childLORef);
				String academicSubject = getByPath(
						"LearningObjectiveIdentity.AcademicSubject", childLORef);
				String objectiveGradeLevel = getByPath(
						"LearningObjectiveIdentity.ObjectiveGradeLevel",
						childLORef);
				...
				if (childNR != null) {
					...
					query.addCriteria(new NeutralCriteria("academicSubject", NeutralCriteria.OPERATOR_EQUAL, academicSubject));
					query.addCriteria(new NeutralCriteria("objectiveGradeLevel", NeutralCriteria.OPERATOR_EQUAL, objectiveGradeLevel));
					...
					if (childEntity != null) {
						...
						setParentObjectiveRef(childEntityNR,
								parentLearningObjRefs);
						...
						super.getErrorReport(parentLO.getSourceFile()).error(
								"Could not resolve LearningObjectiveReference with Objective: "
										+ objective + ", AcademicSubject" + academicSubject
										+ ", ObjectiveGradeLevel" + objectiveGradeLevel, this);
					}
				}
			}
		}
	...
	private void setParentObjectiveRef(NeutralRecord childLo,
			Map<String, Object> childLearningObjRefs) {
		...
		if (!childLo.getAttributes().containsKey(PARENT_LEARNING_OBJ_REF)) {
			...
			super.getErrorReport(childLo.getSourceFile()).error(
					"LearningObjective cannot have multiple parents. "
							+ childLearningObjRefs.toString(), this);
		}
	}

	@SuppressWarnings("unchecked")
	private void moveLearningStdRefsToParentIds(NeutralRecord parentLO) {
		List<Map<String, Object>> childLearningStdRefs = (List<Map<String, Object>>) parentLO
				.getAttributes().get(LEARNING_STD_REFS);

		List<Map<String, Object>> lsRefList = new ArrayList<Map<String, Object>>();
		parentLO.getLocalParentIds()
				.put(LOCAL_ID_LEARNING_STANDARDS, lsRefList);
		...
		if (childLearningStdRefs != null) {
			for (Map<String, Object> learnStdRef : childLearningStdRefs) {
				if (learnStdRef == null) {
					...
					String csn = getByPath(LS_CONTENT_STANDARD_NAME_PATH,
							learnStdRef);
	...
	private void insertTransformedLearningObjectives(
			List<NeutralRecord> transformedLearningObjectives) {
		...
		insertRecords(transformedLearningObjectives,
				LEARNING_OBJECTIVE_TRANSFORMED);
	}
	...
	private static class LearningObjectiveId {
		...
		public int hashCode() {
			...
			result = prime
					* result
					+ ((academicSubject == null) ? 0 : academicSubject
							.hashCode());
			result = prime
					* result
					+ ((objectiveGradeLevel == null) ? 0 : objectiveGradeLevel
							.hashCode());
			return result;
		}

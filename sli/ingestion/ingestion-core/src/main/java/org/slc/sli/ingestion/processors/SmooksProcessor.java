...
public class SmooksProcessor implements Processor {
    ...
    private AbstractIngestionHandler<IngestionFileEntry, FileProcessStatus> smooksParseHandler;
    ...
    public void process(Exchange exchange) throws Exception {
        ...
        if (feWorkNote == null) {
            ...
            processSmooks(exchange, feWorkNote);
        }
    }

    private void processSmooks(Exchange exchange, FileEntryWorkNote feWorkNote) {
        ...
        try {
            ...
            LOG.info("SmooksProcessor: processing {} ", fe.getFileName());
            ...
            // set enchange envelope for handler.
            String owner = fe.getFileType().getName();
            Map<String, Object> envelope = new HashMap<String, Object>();
            envelope.put("headers", exchange.getIn().getHeaders());
            envelope.put("feWorkNote", feWorkNote);
            fe.setEnvelopeByOwner(owner, envelope);
            batchJobDAO.saveBatchJob(newJob);
            envelope = fe.getEnvelopeByOwner(owner);
            // Convert XML file to neutral records, and stage.
            FileProcessStatus fileProcessStatus = smooksParseHandler.handle(fe, databaseMessageReport, rs);
    ...
    private void processMetrics(Metrics metrics, FileProcessStatus fileProcessStatus, ReportStats rs) {
    ...
    public void setSmooksParseHandler(AbstractIngestionHandler<IngestionFileEntry, FileProcessStatus> smooksParseHandler) {
        this.smooksParseHandler = smooksParseHandler;
    }

...
public class DidSchemaParser implements ResourceLoaderAware {
    ...
    private static final String SLI_XSD_NAMESPACE = "SLI";
    ...
    // TODO common code from xsdToNeutalSchemaRepo
            ...
            schemaCollection.setSchemaResolver(new URIResolver() {
                ...
                public InputSource resolveEntity(String targetNamespace, String schemaLocation, String baseUri) {
                    if (resourceLoader != null) {
                        Resource resource = resourceLoader.getResource(xsdPath + "/" + schemaLocation);
                        if (resource.exists()) {
                            try {
                    ...
                    return new InputSource(Thread.currentThread().getContextClassLoader()
                            .getResourceAsStream(xsdPath + "/" + schemaLocation));
                }
            });
        ...
        for (int i = 0; i < numElements; i++) {
            ...
            if (schemaObject instanceof XmlSchemaComplexType) {
                ...
                if (baseName != null && baseName.getLocalPart().equals(REFERENCE_TYPE)) {
                    ...
                    System.out.println("reftype: " + elementTypeName);
                } else {
     ...
     */
    private void extractEntityConfigs() {
        entityConfigs = new HashMap<String, DidEntityConfig>();
     ...
     * Extract ref configs
     */
    private void extractRefConfigs() {
        refConfigs = new HashMap<String, DidRefConfig>();
        ...
        for (Entry<String, XmlSchemaComplexType> refType : referenceTypes.entrySet()) {
            ...
            if (refConfig != null) {
                refConfigs.put(refType.getKey(), refConfig);
            }
        }
    }
    ...
    private DidRefConfig extractRefConfig(XmlSchemaComplexType refType) {
        ...
        // find the identity type - loop over sequence until it is found
        XmlSchemaElement identityTypeElement = null;
        try {
            XmlSchemaComplexContentExtension complexContent = (XmlSchemaComplexContentExtension) refType
                    .getContentModel().getContent();
            identityTypeElement = parseParticleForIdentityType(complexContent.getParticle());
        } catch (Exception e) {
            LOG.error("Failed to extract identityType for " + refType.getName(), e);
        }
        ...
        if (identityTypeElement != null) {
            ...
            parseParticleForKeyFields(identityType.getParticle(), keyFieldMaps, baseXPath);
        ...
        while (!nestedMaps.isEmpty()) {
            ...
            for (Map<String, String> keyFieldMap : keyFieldMaps) {
                ...
                if (parentNode.getRefConfig() == null) {
                    ...
                    nestedRefConfig.setEntityType(parentRefType);
            ...
            for (DidRefConfig refConfig : refConfigList) {
                for (KeyFieldDef keyField : refConfig.getKeyFields()) {
                    ...
                    if (keyField.getKeyFieldName().equals(parentKeyFieldName) &&
                            refConfig.getEntityType().equals(parentRefType)) {
                        return keyField;
                    }
    ...
    private DidEntityConfig extractEntityConfig(XmlSchemaComplexType complexType) {
        ...
        parseParticleForRef(particle, refSources);
        ...
        if (particle != null) {
            if (particle instanceof XmlSchemaElement) {
                ...
                if (elementType != null && referenceTypes.containsKey(elementType.getLocalPart())) {
                    ...
                    if (elementType.getLocalPart().equals("SectionReferenceType")) {

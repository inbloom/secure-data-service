...
public class StudentDisciplineIncidentAssociationTransformer extends AbstractTransformationStrategy {
   ...
   private void transform() {
       try {
           ...
           for (Map.Entry<Object, NeutralRecord> neutralRecordEntry : collection.entrySet()) {
               ...
               String incidentIdentifier = (String) PropertyUtils.getProperty(disciplineIncidentRef, "attributes.IncidentIdentifier");
   ...
   private NeutralRecord findDisciplineIncidentNRFromIdRef(String idRef) {
       ...
       List<NeutralRecord> data = (List<NeutralRecord>) getNeutralRecordMongoAccess().
               getRecordRepository().findByQuery("disciplineIncident", query, 0, 0);
       if (data.size() == 1) {
           result = data.get(0);
       }
       return result;

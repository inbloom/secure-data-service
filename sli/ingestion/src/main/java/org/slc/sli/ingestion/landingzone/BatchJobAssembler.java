...
public class BatchJobAssembler {
	...
	/**
	 * Attempt to generate a new BatchJob based on data found in the controlFile
	 * 
	 * @param controlFile
	 * @return BatchJob
	 * @throws IOException
	...
	public BatchJob assembleJob(ControlFile controlFile) throws IOException {
		...
		if (entries.size()<1) {
			job.addFault(Fault.createError(
					"No files specified in control file"));
		}
		...
		for (ControlFile.FileEntry entry : entries) {
			...
			if (f==null) {
				// file does not exist.
				job.addFault(Fault.createError(
						"File [" + entry.fileName + "] was not found."));
				continue;
			}

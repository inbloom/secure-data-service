...
public class SectionGracePeriodNodeFilter extends NodeFilter {
    ...
    public List<Entity> filterEntities(List<Entity> toResolve, String referenceField) {
        ...
        if (!toResolve.isEmpty() && !endDate.isEmpty()) {
            ...
            Iterable<Entity> sections = helper.getReferenceEntities(EntityNames.SECTION, ID,
                    getReferencedIds(toResolve, referenceField));
            // get the session Ids
            Set<String> sessionIds = getIds(sections, ParameterConstants.SESSION_ID);

            NeutralQuery query = new NeutralQuery();
            query.addCriteria(new NeutralCriteria(ID, NeutralCriteria.CRITERIA_IN, new ArrayList<String>(sessionIds)));
            ...
            return getFilteredEntities(toResolve,
                    getEntityIds(sections, ParameterConstants.SESSION_ID, getIds(sessions, ID)), referenceField);
        }

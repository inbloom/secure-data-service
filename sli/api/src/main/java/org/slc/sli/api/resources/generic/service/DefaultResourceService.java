...
public class DefaultResourceService implements ResourceService {
	...
	protected static interface ServiceLogic {
		public ServiceResponse run(final Resource resource,
				EntityDefinition definition);
	}

	protected ServiceResponse handle(final Resource resource, ServiceLogic logic) {
		EntityDefinition definition = resourceHelper
				.getEntityDefinition(resource);
	...
	@Override
	public ServiceResponse getEntitiesByIds(final Resource resource,
			final String idList, final URI requestURI) {
		...
		return handle(resource, new ServiceLogic() {
			...
			public ServiceResponse run(final Resource resource,
				...
				if (idLength > MAX_MULTIPLE_UUIDS) {
					String errorMessage = "Too many GUIDs: " + idLength
							+ " (input) vs " + MAX_MULTIPLE_UUIDS
							+ " (allowed)";
				...
				ApiQuery apiQuery = resourceServiceHelper.getApiQuery(
				...
				try {
					finalResults = logicalEntity.getEntities(apiQuery,
							resource.getResourceType());
				} catch (UnsupportedSelectorException e) {
					finalResults = (List<EntityBody>) definition.getService()
							.list(apiQuery);
				}
	...
	@Override
	public ServiceResponse getEntities(final Resource resource,
			final URI requestURI, final boolean getAllEntities) {
		...
		return handle(resource, new ServiceLogic() {
			...
			public ServiceResponse run(final Resource resource,
				...
				final ApiQuery apiQuery = resourceServiceHelper.getApiQuery(
						definition, requestURI);
				addAdditionalCriteria(apiQuery);

				if (getAllEntities) {
					entityBodies = SecurityUtil
							.sudoRun(new SecurityUtil.SecurityTask<Iterable<EntityBody>>() {

								@Override
								public Iterable<EntityBody> execute() {
									Iterable<EntityBody> entityBodies = null;
									try {
										entityBodies = logicalEntity
												.getEntities(apiQuery, resource
														.getResourceType());
									} catch (UnsupportedSelectorException e) {
										entityBodies = definition.getService()
												.list(apiQuery);
									}
					...
					try {
						entityBodies = logicalEntity.getEntities(apiQuery,
								resource.getResourceType());
					} catch (UnsupportedSelectorException e) {
				...
				return new ServiceResponse((List<EntityBody>) entityBodies,
						count);
			}
		});
	}
	...
	@Override
	public CalculatedData<Map<String, Integer>> getAggregateData(
	...
	@Override
	public ServiceResponse getEntities(final Resource base, final String id,
			final Resource resource, final URI requestURI) {
		final EntityDefinition definition = resourceHelper
				.getEntityDefinition(resource);

		if (isAssociation(base)) {
			final String key = "_id";
			return getAssociatedEntities(base, base, id, resource, key,
					requestURI);
		}
		...
		final ApiQuery apiQuery = resourceServiceHelper.getApiQuery(definition,
				requestURI);

		// Mongo blows up if we have multiple $in or equal criteria for the same
		// key.
		// To avoid that case, if we do have duplicate keys, set the value for
		// that
		...
		for (NeutralCriteria crit : apiQuery.getCriteria()) {
			if (crit.getKey().equals(associationKey)
					&& (crit.getOperator().equals(NeutralCriteria.CRITERIA_IN) || crit
							.getOperator().equals(
		...
		if (!skipIn) {
			apiQuery.addCriteria(new NeutralCriteria(associationKey, "in",
					valueList));
		}

		try {
			entityBodyList = logicalEntity.getEntities(apiQuery,
					definition.getResourceName());
		} catch (final UnsupportedSelectorException e) {
			entityBodyList = (List<EntityBody>) definition.getService().list(
					apiQuery);
		}
	...
	@Override
	public ServiceResponse getEntities(Resource base, String id,
			Resource association, Resource resource, URI requestUri) {
		final String associationKey = getConnectionKey(base, association);
		return getAssociatedEntities(base, association, id, resource,
				associationKey, requestUri);
	}

	private boolean isAssociation(Resource base) {
		boolean isAssociation = false;

		final EntityDefinition baseEntityDef = resourceHelper
				.getEntityDefinition(base);
		ClassType baseEntityType = provider.getClassType(StringUtils
				.capitalize(baseEntityDef.getType()));
	...
	private ServiceResponse getAssociatedEntities(final Resource base,
			final Resource association, final String id,
			final Resource resource, final String associationKey,
			final URI requestUri) {
		List<String> valueList = Arrays.asList(id.split(","));
		final List<String> filteredIdList = new ArrayList<String>();

		final EntityDefinition finalEntity = resourceHelper
				.getEntityDefinition(resource);
		final EntityDefinition assocEntity = resourceHelper
				.getEntityDefinition(association);
		final EntityDefinition baseEntity = resourceHelper
				.getEntityDefinition(base);

		final String resourceKey = getConnectionKey(association, resource);
		String key = "_id";

		String parentType = EmbeddedDocumentRelations
				.getParentEntityType(assocEntity.getType());
		if ((parentType != null) && baseEntity.getType().equals(parentType)) {
			final ApiQuery apiQuery = resourceServiceHelper
					.getApiQuery(baseEntity);
			apiQuery.setLimit(0);
			...
			for (EntityBody entityBody : baseEntity.getService().list(apiQuery)) {
				...
				List<EntityBody> associations = (List<EntityBody>) entityBody
						.get(assocEntity.getType());
			...
			final ApiQuery apiQuery = resourceServiceHelper
					.getApiQuery(assocEntity);
			apiQuery.setLimit(0);
			apiQuery.addCriteria(new NeutralCriteria(associationKey, "in",
					valueList));
			if (association.getResourceType().equals(
					ResourceNames.STUDENT_SCHOOL_ASSOCIATIONS)
					&& requestUri
							.getPath()
							.matches(
									"^/api/rest/[^/]+/schools/[^/]+/studentSchoolAssociations/students")) {
				apiQuery.addOrQuery(new NeutralQuery(new NeutralCriteria(
						ParameterConstants.EXIT_WITHDRAW_DATE,
						NeutralCriteria.CRITERIA_EXISTS, false)));
				apiQuery.addOrQuery(new NeutralQuery(new NeutralCriteria(
						ParameterConstants.EXIT_WITHDRAW_DATE,
						NeutralCriteria.CRITERIA_GTE, DateTime.now().toString(
								DateTimeFormat.forPattern("yyyy-MM-dd")))));
			}

			for (EntityBody entityBody : assocEntity.getService()
					.list(apiQuery)) {
		...
		List<EntityBody> entityBodyList;
		final ApiQuery finalApiQuery = resourceServiceHelper.getApiQuery(
				finalEntity, requestUri);
		...
		if (!skipIn) {
			finalApiQuery.addCriteria(new NeutralCriteria(key, "in",
					filteredIdList));
		}

		try {
			entityBodyList = logicalEntity.getEntities(finalApiQuery,
					finalEntity.getResourceName());
		} catch (final UnsupportedSelectorException e) {
			entityBodyList = (List<EntityBody>) finalEntity.getService().list(
					finalApiQuery);
		}
	...
	private boolean finalEntityReferencesAssociation(
			EntityDefinition finalEntity, EntityDefinition assocEntity,
	...
	private String getConnectionKey(final Resource fromEntity,
			final Resource toEntity) {
		final EntityDefinition toEntityDef = resourceHelper
				.getEntityDefinition(toEntity);
		final EntityDefinition fromEntityDef = resourceHelper
				.getEntityDefinition(fromEntity);

		ClassType fromEntityType = provider.getClassType(StringUtils
				.capitalize(fromEntityDef.getType()));
		ClassType toEntityType = provider.getClassType(StringUtils
				.capitalize(toEntityDef.getType()));
	...
	protected List<EntityBody> injectErrors(EntityDefinition definition,
			final List<String> ids, List<EntityBody> finalResults) {
		...
		if (idLength > 1) {
			Collections.sort(finalResults, new Comparator<EntityBody>() {
				...
				public int compare(EntityBody o1, EntityBody o2) {
					return ids.indexOf(o1.get("id"))
							- ids.indexOf(o2.get("id"));
				}
			});
			...
			for (int i = 0; finalResultsSize != idLength && i < idLength; i++) {
				...
				try {
					checkedIdMissing = !(finalResults.get(i).get("id")
							.equals(checkedId));
				} catch (IndexOutOfBoundsException ioobe) {
					checkedIdMissing = true;
				}

				// if a particular input ID is not present in the results at the
				// appropriate
				// spot
				if (checkedIdMissing) {
					...
					// try individual lookup to capture specific error message
					// (type)
					try {
						definition.getService().get(ids.get(i));
					} catch (EntityNotFoundException enfe) {
						errorResult.put("type", "Not Found");
						errorResult.put("message", "Entity not found: "
								+ checkedId);
						...
						errorResult.put("message",
								"Access DENIED: " + ade.getMessage());
						errorResult.put("code",
								Response.Status.FORBIDDEN.getStatusCode());
					} /*
					 * catch (Exception e) { errorResult.put("type",
					 * "Internal Server Error"); errorResult.put("message",

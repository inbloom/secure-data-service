...
public class RealmHelper {
	...
	public String getAdminRealmId() {
		Entity realm = SecurityUtil
				.runWithAllTenants(new SecurityTask<Entity>() {

					@Override
					public Entity execute() {
						NeutralQuery realmQuery = new NeutralQuery();
						realmQuery.addCriteria(new NeutralCriteria(
								"uniqueIdentifier",
...
//	}
	...
	public Entity getRealmFromSession(String sessionId) {
		...
		NeutralQuery sessionQuery = new NeutralQuery(new NeutralCriteria("_id",
				NeutralCriteria.OPERATOR_EQUAL, sessionId));
	...
	public Iterable<Entity> getRealms(Entity edOrg) {
		...
		query.addCriteria(new NeutralCriteria("edOrg", "=", edOrg.getBody()
	...
	public boolean isUserAllowedLoginToRealm(Entity userEntity, Entity realm) {
		...
		// Preferred login realms are realms the user would be directly
		// associated with
	...
	private boolean isValidForLogin(Entity edOrgEntity, Entity realm) {
		...
		for (String parentId : edOrgIds) {
			Iterable<Entity> realmEnts = getRealms(repo.findById(
					"educationOrganization", parentId));
	...
	public Set<String> getAssociatedRealmIds() {
		...
		if (isSandboxEnabled) {
			...
			realmQuery
					.addCriteria(new NeutralCriteria("tenantId",

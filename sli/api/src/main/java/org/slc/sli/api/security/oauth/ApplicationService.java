...
public class ApplicationService {
    ...
    private static final String[] ATTRIBUTES_TO_HIDE = new String[] {
            "client_type", "redirect_uri", "name", "developer_info", "client_secret", "description"};
    ...
    public List<EntityBody> getApplications(@Context UriInfo info) {
        ...
        for (String id : realmList) {
            ...
            result.put("link", info.getBaseUri() + info.getPath().replaceAll("/$", "") + "/" + result.get("client_id"));
            results.add(result);
        }
        return results;
    }
    ...
    /**
     * Looks up a specific application based on client ID, ie. /api/rest/apps/<client_id>
     * @param clientId the client ID, not the "id"

...
public class SecurityEventContextResolver implements EntityContextResolver {
	...
	List<String> ROLES_SEA_OR_REALM_ADMIN = Arrays.asList(RoleInitializer.SEA_ADMINISTRATOR, 
			                                              RoleInitializer.REALM_ADMINISTRATOR);
	List<String> ROLES_LEA_ADMIN          = Arrays.asList(RoleInitializer.LEA_ADMINISTRATOR);
	...
	public boolean canResolve(String fromEntityType, String toEntityType) {
		if(toEntityType.equals("securityEvent")) 
			return true;
		else
			return false;
	} 
			...
			if(principal != null) {
				...
				if(roles != null) {
					if (roles.contains(RoleInitializer.SLC_OPERATOR)) {
						...
						} else warn ("Could not find edOrgs for SecurityEvents!");			
					}
				} else warn("Could not find roles for SecurityEvents!");
			} else warn("Could not find principal for SecurityEvents!");
		} else warn("Could not find auth for SecurityEvents!");
		return filters;
	}
}

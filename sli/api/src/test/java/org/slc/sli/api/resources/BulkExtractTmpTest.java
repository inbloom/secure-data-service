...
public class BulkExtractTmpTest {
    ...
    @Autowired
    @InjectMocks
    private BulkExtractTmp bulkExtract;
  ...
  public void testFailedEvaluatePreconditions() throws Exception {
    ...
    public void testGetDelta() throws Exception {
        ...
        try {
            body.put(BulkExtractTmp.BULK_EXTRACT_FILE_PATH, f.getAbsolutePath());
            body.put(BulkExtractTmp.BULK_EXTRACT_DATE, "Sun Apr 22 11:00:00 GMT 2013");
            ...
            when(
                    mockMongoEntityRepository.findOne(eq(BulkExtractTmp.BULK_EXTRACT_FILES),
    ...
    public void testGetLEAListSuccess() throws Exception {
        ...
        Mockito.when(mockMongoEntityRepository.findOne(Mockito.eq(BulkExtractTmp.BULK_EXTRACT_FILES), Mockito.any(NeutralQuery.class)))
    ...
    private void mockBulkExtractEntity() throws IOException {
        ...
        Mockito.when(mockBody.get(BulkExtractTmp.BULK_EXTRACT_FILE_PATH)).thenReturn(inputFile.getAbsolutePath());
        Mockito.when(mockBody.get(BulkExtractTmp.BULK_EXTRACT_DATE)).thenReturn(new Date());
        Mockito.when(mockMongoEntityRepository.findOne(Mockito.eq(BulkExtractTmp.BULK_EXTRACT_FILES), Mockito.any(NeutralQuery.class)))
                .thenReturn(mockEntity);
    }

...
public class StaffEdOrgStaffIDNodeFilterTest {
    private static final String STAFF_REF = "staffReference";

    @InjectMocks
    @Spy
    StaffEdOrgStaffIDNodeFilter nodeFilter = new StaffEdOrgStaffIDNodeFilter(); //class under test
    ...
    public void testFilterIds() {
        ...
        when(mockHelper.getReferenceEntities(eq(EntityNames.STAFF_ED_ORG_ASSOCIATION),
                eq(STAFF_REF), any(List.class))).thenReturn(edorgAssociations);

        when(mockHelper.getReferenceEntities(eq(EntityNames.TEACHER_SCHOOL_ASSOCIATION),
                eq(ParameterConstants.TEACHER_ID), any(List.class))).thenReturn(schoolAssociations);

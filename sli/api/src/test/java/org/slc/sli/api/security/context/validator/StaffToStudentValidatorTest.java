...
public class StaffToStudentValidatorTest {
    ...
    public void testCanGetAccessThroughProgram() {
        Mockito.when(
                mockProgramValidator.validate(Mockito.eq(EntityNames.PROGRAM), Mockito.anySet())).thenReturn(true);
    ...
    public void testCanGetAccessThroughCohort() {
        Mockito.when(
                mockCohortValidator.validate(Mockito.eq(EntityNames.COHORT), Mockito.anySet())).thenReturn(true);
        helper.generateStaffEdorg(STAFF_ID, "DERP", NOT_EXPIRED);
        for (int j = -1; j > -31; --j) {
            String studentId = helper.generateStudentAndStudentSchoolAssociation(String.valueOf(j), ED_ORG_ID,
                    NOT_EXPIRED);
    ...
    public void testCanNotGetAccessThroughExpiredProgram() {
        Mockito.when(
                mockProgramValidator.validate(Mockito.eq(EntityNames.PROGRAM), Mockito.anySet()))
                .thenReturn(false);
        helper.generateStaffEdorg(STAFF_ID, "DERP", NOT_EXPIRED);
        for (int j = -1; j > -31; --j) {
            String studentId = helper.generateStudentAndStudentSchoolAssociation(String.valueOf(j), ED_ORG_ID,

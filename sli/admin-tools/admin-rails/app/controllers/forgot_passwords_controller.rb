...
class ForgotPasswordsController < ApplicationController
  ...
  def get_user
    ...
    if !@user
      flash[:error] = "Unable to verify user. Please contact the inBloom"
    ...
    respond_to do |format|
      if captcha_valid == false
        ...
        begin
          ...
          @forgot_password.errors.add(:base, "Unable to reset your password. Please contact the inBloom.")
          format.html { render action: "reset" }
          format.json { render json: @forgot_password.errors, status: :unprocessable_entity }
        end
      else
        @forgot_password.errors.add(:base, "Unable to verify your user ID. Please contact the inBloom.")
        format.html { render action: "reset" }
        format.json { render json: @forgot_password.errors, status: :unprocessable_entity }
      end
    end

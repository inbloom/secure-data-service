defaultRights = ["READ_GENERAL", "WRITE_GENERAL", "READ_RESTRICTED", "WRITE_RESTRICTED", "READ_STUDENT_GENERAL", "WRITE_STUDENT_GENERAL", "READ_STUDENT_OWNED", "WRITE_STUDENT_OWNED", "READ_STUDENT_RESTRICTED", "WRITE_STUDENT_RESTRICTED", "AGGREGATE_READ", "AGGREGATE_WRITE", "READ_PUBLIC", "WRITE_PUBLIC", "STAFF_CONTEXT", "TEACHER_CONTEXT", "BULK_EXTRACT", "APP_AUTHORIZE", "SECURITY_EVENT_VIEW", "INGESTION_LOG_VIEW"]
selfRights = ["READ_GENERAL", "WRITE_GENERAL", "READ_RESTRICTED", "WRITE_RESTRICTED", "READ_STUDENT_GENERAL", "WRITE_STUDENT_GENERAL", "READ_STUDENT_OWNED", "WRITE_STUDENT_OWNED", "READ_STUDENT_RESTRICTED", "WRITE_STUDENT_RESTRICTED"]
ROLE_COL = "td:eq(0)";
RIGHT_COL = "td:eq(1)";
...
createLabel = (type, name) ->
  label = $('#labelUi').clone()
  if(type == 'role')
    label.find("span").addClass("label-success")
  else if (type =="right")
    label.find("span").addClass("label-default")
  else if (type =="self-right")
    label.find("span").addClass("label-info")
...
getRights = (tr) ->
    rights = []
    tr.find(RIGHT_COL).find(".label-default").each ->
      rights.push($(@).text())
    return rights

getSelfRights = (tr) ->
    rights = []
    tr.find(SELF_RIGHT_COL).find(".label-info").each ->
      rights.push($(@).text())
    return rights

getRoles = (tr) ->
    roles = []
    tr.find(ROLE_COL).find(".label-success").each ->
      roles.push($(@).text())
    return roles
...
getAllRoles = () ->
  roles = []
  $("#custom_roles tr:gt(0)").each ->
    $(@).find(ROLE_COL).find(".label-success").each ->
      roles.push($(@).text())
  return roles

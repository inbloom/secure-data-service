...
grep -lR "https://ci.slidev.org" acceptance-tests/test/data/application_fixture.json | xargs -L 1 sed -n e "s/https:\/\/$1.slidev.org/p"

#Take care of rails projects
echo "Altering rails applications to match..."
grep -lR "https://ci.slidev.org" admin-tools/admin-rails/config/config.yml | xargs -L 1 sed -n e "s/https:\/\/$1.slidev.org/p"
grep -lR "https://ci.slidev.org" databrowser/config/config.yml | xargs -L 1 sed -n e "s/https:\/\/$1.slidev.org/p"

...
def run_fixer(tenant = nil, systemDatabaseName)

  # Tenant Databases are identified by field body.tenantId in records located in sli.tenant collection
  systemDatabaseConnection = Mongo::Connection.new(@hp[0], @hp[1].to_i)
  @systemDatabase = systemDatabaseConnection[systemDatabaseName]
  @tenantCollection = @systemDatabase.collection('tenant')
  
  tenantDatabaseName = ""
  @log.info "--- running tenant stamper for " + tenant + " ---"
  @tenantCollection.find({'body.tenantId' => tenant})  do |row|
    row.each do |systemTenant|
        # database name is stored as systemTenant['body']['tenantId'].  It will be stored in a separate attribute later on.

        @log.info "--- Tenant with ID <" + tenant + "> located in database <" + systemTenant['body']['dbName'] + ">"
        tenantDatabaseName = systemTenant['body']['dbName']
    end
  end

  connection = Mongo::Connection.new(@hp[0], @hp[1].to_i, :pool_size => 10, :pool_timeout => 25, :safe => {:wtimeout => 500})
  db = connection[tenantDatabaseName]
...
def launch_fixer
  ...
  @db['tenant'].find({}) do |cur|
    cur.each do |tenant| 
      if !@tenants.include? tenant['body']['tenantId']
        ...
        @threads << Thread.new {run_fixer(tenant['body']['tenantId'], @database)}
      end
    end
  end
  ...
  @threads << Thread.new {run_fixer(nil, @database)} if @tenants.empty?
...
if ARGV.count < 1
  ...
  puts "\t dbhost - hostname for mongo system SLI database"
  puts "\t port - port mongo system SLI database is running on (27017 is common)"

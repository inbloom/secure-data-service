	...
	private void loadAndStoreEdOrgs(JsonElement root)
	{
		if(root.isJsonArray()){
			...
			for(int i = 0; i < edOrgsJson.size(); i++){
				...
				EducationalOrganization edOrg = new EducationalOrganization(edOrgName, street, suite, city, state, postalCode, stateOrgId);
				edOrg.setType(type);
				edOrgs.put(id, edOrg);
			}
		...
		if(root.isJsonArray()){
			...
			for(int i = 0; schoolsJson != null && i < schoolsJson.size(); i++){
				...
				for(int k = 0; coursesJson != null && k < coursesJson.size(); k++){
					...
					Course course = new Course(courseDefinedBy, adoptionDate, description, characteristics, required, code, gpaAppl,
							idSystem, minCredit, maxCredit, subject, level, courseName, parts, orgCode, careerPath);
	...
	private void loadAndStoreAssessments(JsonElement root)
	{
		/*if(root.isJsonArray()){
			...
			for(int i = 0; i < assessmentsJson.size(); i++){
				...
				String subject = "Other", category = "Achievement test", gradeLevel = "Ungraded", standard = "LEA_STANDARD", version = "1";
				
				AssessmentBody body = new AssessmentBody(name, codes, subject, category, gradeLevel, standard, version);
	...
	private void loadAndStoreStudents(JsonElement root)
	{
		if(root.isJsonArray()){
			...
			for(int i = 0; i < studentsJson.size(); i++){
				...
				String uid = getString(studentJson, "uid", getString(studentJson, "studentUniqueStateId", getString(studentJson, "stateId", getString(studentJson, "id", Base64.nextUuid(Student.typeCode)))));
				...
				boolean hispanicLatinoEthnicity = Boolean.parseBoolean(getString(studentJson, "hispanicLatinoEthnicity", "false"));

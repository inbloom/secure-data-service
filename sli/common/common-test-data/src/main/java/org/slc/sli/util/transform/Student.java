...
public class Student implements MongoDataEmitter 
{
	...
	public Student(StudentName name, BirthData birth, String stateId, String sex, String econDis, School school, String grade, String sfse, String stuChar, 
			String lep, String sect504Dis, String progPartic, String displacement, String disability)
	{
		this.name = name;
	...
	public String emitXml()
	{
		...
		if(name != null){
			...
			if(name.getSuffix() != null){
				answer.append("      <GenerationCodeSuffix>").append(name.getSuffix()).append("</GenerationCodeSuffix>\n");
			}
		}
		answer.append("   </Name>\n");
		...
		answer.append("      <TelephoneNumber>").append(name.getFirstName()).append(".").append(name.getLastSurname()).append("@example.com").append("</TelephoneNumber>\n");
		answer.append("   </ElectronicMail>\n");
		answer.append("   <HispanicLatinoEthnicity>").append(hispanicLatinoEthnicity).append("</HispanicLatinoEthnicity>\n");
		answer.append("   <Race>\n");
		...
		answer.append("   <SchoolFoodServicesEligibility>").append(schoolFoodServicesEligibility).append("</SchoolFoodServicesEligibility>\n");
		...
		if(section504Disabilities != null && section504Disabilities.length() > 0){
			answer.append("      <Section504Disability>").append(section504Disabilities).append("</Section504Disability>\n");
		}
		answer.append("   </Section504Disabilities>\n");
	...
	public String emit()
	{
		...
		StringBuffer answer = new StringBuffer();
		answer.append("{\"_id\":{\"$binary\":\"").append(Base64.toBase64(generatedUuid)).append("\",\"$type\":\"03\"},\"type\":\"student\",\"body\":{\"studentUniqueStateId\":\"").
			append(stateStudentId).append("\",").append(name.emit()).append(",\"sex\":\"").append(sex).append("\",\"economicDisadvantaged\":").append(econDis).
			append(",\"schoolFoodServicesEligibility\":\"").append(schoolFoodServicesEligibility).append("\",\"hispanicLatinoEthnicity\":").append(hispanicLatinoEthnicity).
			append(",\"race\":\"").append(racialCategory).append("\",");
		if(studentCharacteristics != null && studentCharacteristics.length() > 0){
			answer.append("\"studentCharacteristics\":[{\"characteristic\":\"").append(studentCharacteristics).append("\"}],");
		}
		if(disability != null && disability.length() > 0){
	...
	public String emitSchoolAssociation()
	{
		...
		if(Configuration.outputType.equals(Configuration.OutputType.Fixture)){
			answer.append("{\"_id\":{\"$binary\":\"").append(Base64.toBase64(generatedStudentSchoolAssociationUuid)).append("\",\"$type\":\"03\"},\"type\":\"studentSchoolAssociation\",\"").
				append("body\":{\"entryGradeLevel\":\"").append(gradeLevel).append("\",\"studentId\":\"").append(generatedUuid).append("\",\"schoolId\":\"").
				append(school.getUuid()).append("\"}}\n");
		}else{
			/*
	<StudentSchoolAssociation>
        ...
        <EducationalPlans>
			...
			answer.append("         <StateOrganizationId>").append(school.getStateUniqueId()).append("</StateOrganizationId>\n");
			answer.append("      </EducationalOrgIdentity>\n");
			answer.append("   </SchoolReference>\n");
	...
	public String emitSectionAssociations()
	{
		...
		while(iter.hasNext()){
			...
			answer.append("{\"_id\":{\"$binary\":\"").append(Base64.toBase64(Base64.nextUuid(sectionAssocTypeCode))).append("\",\"$type\":\"03\"},\"type\":\"studentSectionAssociation\",\"").
				append("body\":{\"repeatIdentifier\":\"Not repeated\",\"studentId\":\"").append(generatedUuid).append("\",\"sectionId\":\"").append(sectionId).
	...
	{
		generatedUuid = id;	// okay, so it isn't really generated at this point.  This helps to keep dashboard IDs predictable.
	}

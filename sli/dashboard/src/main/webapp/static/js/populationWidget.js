/**
 * this script populates drop-down menu for Educational Organization for LOS
 */

var Dashboard = {
	populateInstHierarchy : function() {
		...
		for (i = 0; i < instHierarchy.length; i++) {
			y += "<option value=\"" + i + "\">" + instHierarchy[i].name
					+ "</option>"
		}
		y += "</select>"
	...
	populateSchoolMenu : function(edorgIndex) {
		var temp = instHierarchy[edorgIndex].schools

		var y = "<select id=\"schoolSelect\" onChange=\"populateCourseMenu("
				+ edorgIndex + ",this.value)\">"
		y += "<option value=\"-1\"></option>"
		var i = 0;
		for (i = 0; i < temp.length; i++) {
			y += "<option value=\"" + i + "\">" + temp[i].nameOfInstitution
					+ "</option>"
		}
		y += "</select>"
	...
	populateCourseMenu : function(edorgIndex, schoolIndex) {
		var temp = instHierarchy[edorgIndex].schools[schoolIndex].courses

		var y = "<select id=\"courseSelect\" onChange=\"populateSectionMenu("
				+ edorgIndex + "," + schoolIndex + ",this.value)\">"
		y += "<option value=\"\"></option>"
		var j = 0;
		for (j = 0; j < temp.length; j++) {
			y += "<option value=\"" + j + "\">" + temp[j].courseTitle
					+ "</option>"
		}
		document.getElementById("courseDiv").innerHTML = y
	},

	populateSectionMenu : function(edorgIndex, schoolIndex, courseIndex) {
		var temp = instHierarchy[edorgIndex].schools[schoolIndex].courses[courseIndex].sections
		var y = "<select id=\"sectionSelect\" onChange=\"printStudentList("
				+ edorgIndex + "," + schoolIndex + "," + courseIndex
				+ ", this.value, 0)\">"
		y += "<option value=\"\"></option>"
		var i = 0
		for (; i < temp.length; i++) {
			y += "<option value=\"" + i + "\">" + temp[i].sectionName
					+ "</option>"
		}
		y += "</select>"
	...
	changeView : function(viewIndex) {
		...
		printStudentList(edOrgIndex, schoolIndex, courseIndex, sectionIndex,
				viewIndex, filterIndex);
	},

	filterStudents : function(filterIndex) {
		var edOrgIndex = document.getElementById("edOrgSelect").value;
		var schoolIndex = document.getElementById("schoolSelect").value;
		var courseIndex = document.getElementById("courseSelect").value;
		var sectionIndex = document.getElementById("sectionSelect").value;
	...
	printStudentList : function(edorgIndex, schoolIndex, courseIndex,
			sectionIndex, viewIndex, filterIndex) {
		var i = 0;
		...
		$("#studentDiv")
				.load(
						...
						function(responseText, textStatus, XMLHttpRequest) {
							...
							if (document
									.getElementById("studentFilterSelector")) {
								document
										.getElementById("studentFilterSelector").selectedIndex = filterIndex;
							}
						});
	},

	studentProfilePopup : function(id) {

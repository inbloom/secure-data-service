...
public abstract class GenericLayoutController {
     ...
     * @param layoutId - unique id of the layout 
     * @param entityId - entity id to pass to the child panels
     * @return
     */
    protected ModelMap getPopulatedModel(String layoutId, Object entityId) {
        ...
        if (viewConfigs == null)
        {
            logger.error("Unable to populate find config for layout" + layoutId + " and user " + SecurityUtil.getUsername());
        ...
        for (ViewConfig viewConfig : viewConfigs)
        {
            if (viewConfig.getDisplaySet() != null)
            {
                for (DisplaySet panel: viewConfig.getDisplaySet())
                {
                    ...
                    {
                      model.addAttribute(
                              panel.getDisplayName(), customizationAssemblyFactory.getDataComponent(panel.getDisplayName(), entityId));
                    }

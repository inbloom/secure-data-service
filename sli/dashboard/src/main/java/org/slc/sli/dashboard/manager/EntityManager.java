...
public class EntityManager extends ApiClientManager {
	 ...
	 */
	public GenericEntity getStudentWithOptionalFields(final String token,
			String studentId, List<String> optionalFields) {
		return getApiClient().getStudentWithOptionalFields(token, studentId,
				optionalFields);
	}
	...
	public GenericEntity getStudentForCSIPanel(final String token,
		...
		if (student == null) {
			throw new DashboardException(
					"Unable to retrieve data for the requested ID");
		}
		student = ContactSorter.sort(student);
		student = GenericEntityEnhancer.enhanceStudent(student);

		GenericEntity section = getApiClient().getSectionHomeForStudent(token,
				studentId);

		if (section != null) {
			student.put(Constants.ATTR_SECTION_ID,
					section.get(Constants.ATTR_UNIQUE_SECTION_CODE));
			GenericEntity teacher = getApiClient().getTeacherForSection(token,
					section.getString(Constants.ATTR_ID));

			if (teacher != null) {
				@SuppressWarnings("unchecked")
				Map<String, Object> teacherName = (Map<String, Object>) teacher
		...
		List<GenericEntity> studentEnrollment = getApiClient()
				.getEnrollmentForStudent(token, student.getId());
		student.put(Constants.ATTR_STUDENT_ENROLLMENT, studentEnrollment);

		List<GenericEntity> parents = getApiClient().getParentsForStudent(
				token, studentId, new HashMap<String, String>());
	 ...
	 */
	public List<GenericEntity> getAttendance(final String token,
			final String studentId, final String start, final String end) {
	 ...
	 * @return
	 */
	public List<GenericEntity> getStudentGradebookEntries(final String token,
			final String studentId, Map<String, String> params) {
		return getApiClient().getCoursesForStudent(token, studentId, params);
	}
	...
	public List<GenericEntity> getStudentsWithGradebookEntries(
		...
		return getApiClient().getStudentsForSectionWithGradebookEntries(token,
				sectionId);
	}
	 ...
	 * @return
	 */
	public List<GenericEntity> getEntities(final String token, final String type,
			final List<String> ids, Map<String, String> params) {
	...
	public GenericEntity getAcademicRecord(String token, String studentId,
			Map<String, String> params) {
		List<GenericEntity> entities = getApiClient()
				.getAcademicRecordsForStudent(token, studentId, params);
	...
	public List<GenericEntity> getAcademicRecords(String token, String studentId,
			Map<String, String> params) {
		List<GenericEntity> entities = getApiClient()
				.getAcademicRecordsForStudent(token, studentId, params);
	...
	public GenericEntity getSectionForProfile(String token, String sectionId,
		...
		if (courseOffering != null) {
			String courseId = courseOffering
					.getString(Constants.ATTR_COURSE_ID);
			if (courseId != null) {
				...
				if (course != null) {
					section.put(Constants.ATTR_COURSE_TITLE,
							course.get(Constants.ATTR_COURSE_TITLE));
					section.put(Constants.ATTR_SUBJECTAREA,
							course.get(Constants.ATTR_SUBJECTAREA));
				}
			}
		}
	...
	private List<GenericEntity> cacheStudent(String token, List<Link> links,
		...
		for (Link link : links) {
			...
			if (link.getLinkName().equals(
					Constants.GET_STUDENT_SECTION_ASSOCIATIONS)) {
		...
		if (courseOfferingIds.size() > 0) {
			entities = getApiClient().readEntityList(token, courseOfferingIds,
					null, SDKConstants.COURSE_OFFERINGS);
...
//
	...
	public GenericEntity getEdorgProfile(String token, String id) {
		...
		GenericEntity edorg = getApiClient().getEducationalOrganization(token,
				id);
		GenericEntity ge = new GenericEntity();

		if (edorg != null) {
			ge.put(Constants.ATTR_NAME_OF_INST,
					edorg.getString(Constants.ATTR_NAME_OF_INST));
			Link link = edorg.getLink(Constants.ATTR_GET_FEEDER_SCHOOLS);
			List<GenericEntity> schools = getApiClient().readEntityList(token,
					link.getResourceURL().toString());
			...
			for (GenericEntity school : schools) {
				...
				tempSchool.put(Constants.ATTR_ID,
						school.getString(Constants.ATTR_ID));
				tempSchool.put(Constants.ATTR_NAME_OF_INST,
						school.getString(Constants.ATTR_NAME_OF_INST));
	...
	public GenericEntity getStateEdorgProfile(String token, String edorgId) {
		...
		if (edorg != null) {
			...
			List<GenericEntity> districts = getApiClient().readEntityList(
					token, link.getResourceURL().toString());
			LinkedList<GenericEntity> districtList = new LinkedList<GenericEntity>();

			for (GenericEntity district : districts) {
				GenericEntity tempSchool = new GenericEntity();
				tempSchool.put(Constants.ATTR_ID,
						district.getString(Constants.ATTR_ID));
				tempSchool.put(Constants.ATTR_NAME_OF_INST,
						district.getString(Constants.ATTR_NAME_OF_INST));
	...
	public GenericEntity getCurrentCoursesAndGrades(String token,
		...
		try {
			...
			for (GenericEntity studentSectionAssociation : studentSectionAssociations) {
				...
				// if
				// (studentSectionAssociation.getString(Constants.ATTR_ID).equals("2012en-03720e8d-e7c7-11e1-b76b-001e4f459459"))
				// {
				...
				if (section != null) {
					toAdd.put(Constants.ATTR_SECTION_NAME,
							section.get(Constants.ATTR_UNIQUE_SECTION_CODE));
					Map teacher = (Map) section
							.get(Constants.ATTR_TEACHER_NAME);

					StringBuilder teacherName = new StringBuilder();
					if (teacher != null) {
						if (teacher
								.containsKey(Constants.ATTR_PERSONAL_TITLE_PREFIX)) {
							teacherName = teacherName
									.append(teacher
											.get(Constants.ATTR_PERSONAL_TITLE_PREFIX))
									.append(". ");
						}
						if (teacher.containsKey(Constants.ATTR_FIRST_NAME)) {
							teacherName = teacherName.append(
									teacher.get(Constants.ATTR_FIRST_NAME))
									.append(" ");
						}
						if (teacher.containsKey(Constants.ATTR_LAST_SURNAME)) {
							teacherName = teacherName.append(teacher
									.get(Constants.ATTR_LAST_SURNAME));
						}
					}
					toAdd.put(Constants.ATTR_TEACHER_NAME,
							teacherName.toString());
					toAdd.put(Constants.ATTR_COURSE_TITLE,
							section.get(Constants.ATTR_COURSE_TITLE));
					toAdd.put(Constants.ATTR_SUBJECTAREA,
							section.get(Constants.ATTR_SUBJECTAREA));
				}
				...
				for (Link stuSecLinks : studentSectionAssociation.getLinks()) {
					if (stuSecLinks.getLinkName().equals(Constants.GET_GRADES)) {
						...
						for (GenericEntity grade : grades) {
							toAdd.put(
									grade.getString(Constants.GRADE_TYPE),

...
/**
 * An class someone didn't think important enough to document
 *TODO document this 
 *
 */
public class CustomizationAssemblyFactoryImpl implements CustomizationAssemblyFactory {
    ...
    public GenericEntity getDataComponent(String componentId, Object entityId) {
        ...
        try {
            Method m = PopulationManager.class.getMethod(getGetterName(config), new Class[] { String.class,
                    Object.class, ViewConfig.class });
            return (GenericEntity) m.invoke(populationManager, getTokenId(), entityId, config);
        } catch (NoSuchMethodException nsme) {
            logger.error("Invalid data component specified for " + componentId + " and entity id " + entityId
                    + ", config " + config, nsme);
            
        } catch (Throwable t) {
            logger.error("Unable to invoke population manager for " + componentId + " and entity id " + entityId
                    + ", config " + config, t);
        }
        return null;
    }

...
public class CustomizationAssemblyFactoryImpl implements CustomizationAssemblyFactory {
    ...
    public GenericEntity getDataComponent(String componentId, Object entityKey) {
        ...
        if (config == null) {
            throw new DashboardException("Unable to find config for " + componentId + " and entity id " + entityKey
                    + ", config " + componentId);
        }
        try {
            ...
            logger.error("Invalid data component specified for " + componentId + " and entity id " + entityKey
                    + ", config " + componentId, nsme);

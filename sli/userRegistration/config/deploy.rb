...
set :application, "User Account registration app"

set :working_dir, "sli/userRegistration"

set :repository,  "git@git.slidev.org:sli/sli.git"
set :bundle_gemfile, "sli/userRegistration/Gemfile"
...
namespace :deploy do
  ...
  task :start do
    ...
    run  <<-CMD
      cd #{deploy_to}/current/sli/userRegistration; bundle exec thin start -C config/thin.yml -e #{rails_env}
    CMD
  end
  ...
  task :stop do
    run  <<-CMD
      cd #{deploy_to}/current/sli/userRegistration; bundle exec thin stop -C config/thin.yml -e #{rails_env}
    CMD
  end

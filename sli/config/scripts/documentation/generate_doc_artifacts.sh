...
#           Codebase recently build from the top level: In SLI_HOME, mvn clean package 
#           API and ingestion servers running
#           apiDocumentationSetup rake task run from SLI_HOME/acceptance-test folder
...
function generateApiGetExamples {
    bundle install
    cd autogenerated/
    ruby ../get_resources.rb $SLI_HOME/api/src/main/resources/wadl/resources.json cacd9227-5b14-4685-babe-31230476cf3b http://local.slidev.org:8080/api/rest http://localhost/api/rest
    cd ..
}
...
function generateDataModelSchema {
    cd autogenerated/
    java -jar `ls $SLI_HOME/modeling/docgen/target/docgen-*-jar-with-dependencies.jar` -outFolder . -outFile schema.xml -xmiFile $SLI_HOME/domain/src/main/resources/sliModel/SLI.xmi -domainFile $SLI_HOME/domain/src/main/resources/sliModel/domains.xml
    cd ..
}

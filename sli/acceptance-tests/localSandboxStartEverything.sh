...
#`activemq start`
`cd #{dir}/config/scripts/; ./resetAllDbs.sh`
`#{dir}/search-indexer/scripts/local_search_indexer.sh start`
...
#  {name: 'MockZIS', port: 8087, dir: "#{dir}/sif/mock-zis", exec: "mvn -o jetty:run", pattern: jetty_pattern},
  ...
  {name: 'AdminTools', port: 3001, dir: "#{dir}/admin-tools/admin-rails", exec: "bundle install; bundle exec rails server -e development_sb", pattern: />> Listening on/},
#  {name: 'SIFAgent', port: 1337, dir: "#{dir}/sif/sif-agent", exec: "mvn -o tomcat:run", pattern: tomcat_pattern},
{name: 'SampleApp', port: 8081, dir: "#{dir}/SDK/sample", exec: "mvn -o jetty:run", pattern: jetty_pattern},

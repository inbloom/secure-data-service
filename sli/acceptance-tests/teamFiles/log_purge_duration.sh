#!/bin/sh
dataset="purge.zip"
start_date=`mongo ingestion_batch_job < times.js | awk {'print $3'} | awk 'NR==23'`
start_date=${start_date:9: 19}
end_date=`mongo ingestion_batch_job < times.js | awk {'print $3'} | awk 'NR==24'`
end_date=${end_date:9: 19}
let diff1=( `date +%s -d $end_date`-`date +%s -d $start_date` )
status=`mongo ingestion_batch_job < times.js | awk {'print $3'} | awk 'NR==25'`

...
Then /^I receive a JSON object listing all the admin apps that my SEA\/LEA have approved$/ do
  ...
  @result = JSON.parse(@res.body)
  assert(@result != nil, "Result of JSON parsing is nil")
  assert(@result.length < 6, "around 2 admin apps but got back #{@result.length}") #important thing is this is less than the result of the size of the list of all apps
end

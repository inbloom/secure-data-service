...
Scenario: Add context rights in a new group
  ...
  And the group "Context Role" contains the "right" rights "Staff Read General"

  When I edit the group "Context Role"
  And I remove the right "STAFF_CONTEXT" from the group "Context Role"
  ...
  Then I am no longer in edit mode
  When I edit the group "Context Role"
  And I should not see the "STAFF_CONTEXT" in the rights dropdown list
  And I wait for "1" seconds
  And I click the cancel button
  Then I am no longer in edit mode
  When I edit the group "Context Role"

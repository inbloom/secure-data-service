...
Scenario: Create a long lived session for user sgoddard
	Given I used the long lived session token generator script to create a token for user "sgoddard" with role "Educator" in tenant "Hyrule"  for realm "NY" that will expire in "300" seconds with client_id "AT1k3PdHzX" 

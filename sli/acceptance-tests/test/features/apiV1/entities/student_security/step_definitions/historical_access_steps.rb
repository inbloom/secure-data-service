...
Then /^I should be able to access data about (the student "[^"]*")$/ do |arg1|
  ...
  ["attendances", "studentAssessments", "reportCards", "studentDisciplineIncidentAssociations", "studentParentAssociations", 'studentGradebookEntries'].each do |endpoint|
...
Then /^I should not be able to access data about (the student "[^"]*")$/ do |arg1|
  @format = "application/vnd.slc+json"
  ["attendances", "studentAssessments", "reportCards", "studentDisciplineIncidentAssociations", "studentParentAssociations"].each do |endpoint|
    restHttpGet("/v1/#{endpoint}?studentId=#{arg1}")

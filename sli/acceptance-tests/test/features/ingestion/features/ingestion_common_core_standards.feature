...
Then I should see following map of entry counts in the corresponding collections:
   ...
   And I check to find if record is in collection:
     ...
     | learningStandard            | 0                   | body.learningStandard.description              | Explain and use the relationship between the sine and cosine of complementary angles. |
     | learningObjective           | 3                   | body.objectiveGradeLevel                       | Twelfth grade                                |
     | learningStandard            | 0                   | body.objectiveGradeLevel                       | Twelfth grade                                |

  And I should see "Processed 3 records." in the resulting batch job file
  And I should not see an error log file created
  And I should see "Grade_12_Math_CCS_G_C.xml records considered: 2" in the resulting batch job file
  And I should see "Grade_12_Math_CCS_G_C.xml records ingested successfully: 2" in the resulting batch job file
  And I should see "Grade_12_Math_CCS_G_C.xml records failed: 0" in the resulting batch job file
  And I should see "Grade_12_Math_CCS_G_SRT.xml records considered: 1" in the resulting batch job file
  And I should see "Grade_12_Math_CCS_G_SRT.xml records ingested successfully: 1" in the resulting batch job file

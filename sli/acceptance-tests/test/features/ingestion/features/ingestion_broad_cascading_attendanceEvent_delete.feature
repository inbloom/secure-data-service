...
@wip
Scenario: Attempt Force delete AttendanceEvent without Section Ref
    Given I am using preconfigured Ingestion Landing Zone for "Midgar-Daybreak"
    And the "Midgar" tenant db is empty
    When the data from "test/features/ingestion/test_data/delete_fixture_data/" is imported
    Then there exist "1" "attendance" records like below in "Midgar" tenant. And I save this query as "attendance"
    ...
    And I check the number of elements in array of collection:
        ...
        | attendance       |_id    |94de66549a6b58f96463ff0d59b34817aa1fead6_id | 22                | body.attendanceEvent |
    And I save the collection counts in "Midgar" tenant
    And I post "AttemptForceAttendanceEventDeleteWithoutSectionRef.zip" file as the payload of the ingestion job
    When zip file is scp to ingestion landing zone
    And a batch job for file "AttemptForceAttendanceEventDeleteWithoutSectionRef.zip" is completed in database
    And I should see "Processed 1 records." in the resulting batch job file
	And I should see "records deleted successfully: 1" in the resulting batch job file
	And I should see "records failed processing: 0" in the resulting batch job file
    And I should not see an error log file created
   	And I should not see a warning log file created
    And I re-execute saved query "attendance" to get "1" records
    And I see that collections counts have changed as follows in tenant "Midgar"
        | collection                                |     delta|
        | attendance                                |         0|
        | attendanceEvent                           |        -1|
...
@wip
Scenario: Attempt Force delete AttendanceEvent by ref without Section Ref
    Given I am using preconfigured Ingestion Landing Zone for "Midgar-Daybreak"
    And the "Midgar" tenant db is empty
    When the data from "test/features/ingestion/test_data/delete_fixture_data/" is imported
    Then there exist "1" "attendance" records like below in "Midgar" tenant. And I save this query as "attendance"
    ...
    And I check the number of elements in array of collection:
        ...
        | attendance       |_id    |94de66549a6b58f96463ff0d59b34817aa1fead6_id | 22                | body.attendanceEvent |
    And I save the collection counts in "Midgar" tenant
    And I post "AttemptForceAttendanceEventRefDeleteWithoutSectionRef.zip" file as the payload of the ingestion job
    When zip file is scp to ingestion landing zone
    And a batch job for file "AttemptForceAttendanceEventRefDeleteWithoutSectionRef.zip" is completed in database
    And I should see "Processed 1 records." in the resulting batch job file
	And I should see "records deleted successfully: 1" in the resulting batch job file
	And I should see "records failed processing: 0" in the resulting batch job file
    And I should not see an error log file created

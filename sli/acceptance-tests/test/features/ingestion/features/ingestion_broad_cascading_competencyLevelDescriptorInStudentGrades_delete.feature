...
Scenario: Delete Competency Level Descriptor with cascade = false and force = true, log violations = true
    ...
    And I post "ForceCompetencyLevelDescriptorInGradeDelete.zip" file as the payload of the ingestion job
	When zip file is scp to ingestion landing zone
    And a batch job for file "ForceCompetencyLevelDescriptorInGradeDelete.zip" is completed in database
...
Scenario: Delete Competency Level Descriptor Reference with cascade = false and force = true, log violations = true
	...
	|_id        |d82250f49dbe4facb59af2f88fe746f70948405d_id     |
    And I save the collection counts in "Midgar" tenant   
    And I post "ForceCompetencyLevelDescriptorRefInGradeDelete.zip" file as the payload of the ingestion job
	When zip file is scp to ingestion landing zone
    And a batch job for file "ForceCompetencyLevelDescriptorRefInGradeDelete.zip" is completed in database
	And I should see "records considered for processing: 1" in the resulting batch job file
	And I should see "records ingested successfully: 0" in the resulting batch job file
	And I should see "records deleted successfully: 1" in the resulting batch job file
	And I should see "records failed processing: 0" in the resulting batch job file

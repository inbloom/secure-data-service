...
Scenario: Delete Objective Assessment From Assessment Metadata with cascade
    ...
    And I post "BroadObjectiveAssessmentFromAssessmentMetadataInterchangeDelete.zip" file as the payload of the ingestion job
  	When zip file is scp to ingestion landing zone
    And a batch job for file "BroadObjectiveAssessmentFromAssessmentMetadataInterchangeDelete.zip" is completed in database
    And a batch job log has been created

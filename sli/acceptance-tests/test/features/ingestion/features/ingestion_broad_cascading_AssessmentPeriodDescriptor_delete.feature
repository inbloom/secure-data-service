...
@wip
Scenario: Delete AssessmentPeriodDescriptor with cascade
	...
	And I re-execute saved query "APD2" to get "0" records
...
Scenario: Delete AssessmentPeriodDescriptor with default settings (Confirm that by default cascade = false, force = true and log violations = true)
    Given I am using preconfigured Ingestion Landing Zone for "Midgar-Daybreak"
    And the "Midgar" tenant db is empty
    When the data from "test/features/ingestion/test_data/delete_fixture_data/" is imported
    Then there exist "1" "assessmentPeriodDescriptor" records like below in "Midgar" tenant. And I save this query as "APD1"
